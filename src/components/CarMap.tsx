...
const createEnhancedCarIcon = (car: CarData): L.DivIcon => {
...
const getCarData = (): CarData[] => {
  ...
  return carsData.map((car, index) => ({
    ...
    lat: car.latitude,
    lng: car.longitude,
    status: car.available ? ('available' as const) : ('rented' as const),
...
const CarMap: React.FC = () => {
  ...
  const [mapCenter] = useState<[number, number]>([25.2048, 55.2708]); // Dubai coordinates
