...
const generateMockData = (): DashboardData => {
  ...
  for (let i = 0; i < 24; i++) {
    ridesData.push({
      ...
      yesterday: Math.floor(Math.random() * 6) + 1,
    });
  }
  ...
  return {
    ...
    pendingProofsCount: Math.floor(Math.random() * 8) + 1,
  };
};
...
const AppContent: React.FC = () => {
  ...
  const [notifications] = useState<Notification[]>([
    { id: 1, message: 'New booking received', time: '2 min ago', unread: true, type: 'booking', title: 'New Booking' },
    { id: 2, message: 'Vehicle maintenance due', time: '1 hour ago', unread: true, type: 'maintenance', title: 'Maintenance Alert' },
    { id: 3, message: 'Payment processed successfully', time: '3 hours ago', unread: false, type: 'payment', title: 'Payment Success' },
  ]);
          ...
          <div className="px-4 sm:px-6 py-3">
            <div className="flex justify-between items-center">
              <div className="flex items-center space-x-3">
                {/* Mobile Menu Button */}
                <button className="lg:hidden p-2 rounded-lg hover:bg-gray-100 transition-colors" onClick={() => setIsMobileOpen(true)}>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600 dark:text-gray-400 hidden sm:block">
                    {activeSection === 'dashboard' ? 'Car Rental Management' : `Manage ${activeSection.replace('-', ' ')}`}
                  </p>
                </div>
              </div>
                    ...
                    <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 py-2 z-50">
                      ...
                      <div className="max-h-64 overflow-y-auto">
                        {notifications.map((notification) => (
                          <div
                            key={notification.id}
                            className={`px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors border-l-2 ${
                              notification.unread
                  ...
                  >
                    ...
                    <div className="hidden sm:block text-left">
                      <p className="text-sm font-medium text-gray-900 dark:text-white">
                        {user?.profile?.firstName ? `${user.profile.firstName} ${user.profile.lastName}` : user?.email || 'User'}
                      </p>
